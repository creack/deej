...
func (sf *wcaSessionFinder) getDefaultAudioEndpoints() (*wca.IMMDevice, *wca.IMMDevice, error) {
	...
	var mmInDevice, mmOutDevice *wca.IMMDevice
...
func (sf *wcaSessionFinder) getMasterSession(mmDevice *wca.IMMDevice, key, loggerKey string) (*masterSession, error) {
...
func (sf *wcaSessionFinder) enumerateAndAddSessions(sessions *[]Session) error {
	...
	for deviceIdx := uint32(0); deviceIdx < deviceCount; deviceIdx++ {
		...
		newSession, err := sf.getMasterSession(endpoint, endpointFriendlyName, fmt.Sprintf(deviceSessionFormat, endpointDescription))
		if err != nil {
...
func (sf *wcaSessionFinder) defaultDeviceChangedCallback(this *wca.IMMNotificationClient, EDataFlow, eRole uint32, lpcwstr uintptr) uintptr {
...
func (sf *wcaSessionFinder) noopCallback() uintptr { return 0 }
